...
Прокрутка ленты — собственная, не системная (`TR-13`). Причина: защёлка
(`TR-29`) требует, чтобы у конца ленты движение замедлялось относительно
пальца, а системная прокрутка всегда идёт один к одному. Защёлка —
фундаментальное требование и не меняется, поэтому физика своя.

Числа физики берутся из опубликованных источников Apple, а не подбираются
на глаз: формула сопротивления из «Designing Fluid Interfaces» с
константой 0.55, границы — пружина с демпфированием 1.0 и откликом 0.4 с,
принимающая скорость жеста: отскок от края и возврат растянутой ленты —
один механизм, глубина отскока следует из скорости. Подбор значений по
ощущениям заказчика отвергнут: он давал то дёрганье, то свободно
катающиеся карточки.

Принято заказчиком 19.08.2026 после исправления передачи: отскок только
на настоящем крае (прямая проверка позиции), зона защёлки пружине не
передаётся, инерция после отпускания растянутой ленты игнорируется.

## Тактильный отклик трея

Щелчок защёлки трея (`TR-29`) отдаётся напрямую в актуатор трекпада: для
фонового приложения строки меню системный тактильный API молчит. Импульс
уходит в тот трекпад, которым сделан жест, и одинаков от раза к разу.

Принятые ограничения, за которые заплачено отладкой 19.08.2026:

1. Доступный импульс ровно один. Ни номер волны, ни параметры вызова на
   выход не влияют — измерено акустически и подтверждено рукой. Поэтому
   вход и выход из защёлки ощущаются одинаково, и разным трекпадам нельзя
   дать разные импульсы.
2. Сила задана железом. Встроенный трекпад отзывается резче внешнего:
   корпус ноутбука работает резонатором, а внешняя пластина гасит энергию
   массой и столом. Средствами этого интерфейса это не выравнивается.
3. Добирать силу несколькими импульсами подряд запрещено — резонансный
   мотор не успевает остановиться, и толчок выходит грязным и каждый раз
   разным.
4. Используются приватные символы системы, поэтому приложение в таком виде
   не пройдёт в Mac App Store. Для локальной сборки цена принята.
