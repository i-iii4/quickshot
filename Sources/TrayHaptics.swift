...
final class TrayHaptics: @unchecked Sendable {
    /// Смысл события. Физику подбирает нижний слой — по устройству, потому
    /// что одинаковая команда на разных трекпадах звучит по-разному.
    /// СЕГОДНЯ ОБА СЛУЧАЯ ОТДАЮТ ОДИН И ТОТ ЖЕ ИМПУЛЬС и физически
    /// неразличимы: доступный импульс ровно один (см. ниже). Разделение
    /// оставлено смысловым — если у интерфейса появится второй различимый
    /// импульс, менять придётся только эту таблицу, а не вызывающий код.
    enum Pulse {
        /// Посадка в защёлку.
        case firm
        /// Выход из защёлки.
    ...
    /// Волна 1 — «слабый щелчок»: сила понижена на ступень по решению
    /// заказчика 19.08.2026 (после устранения паразитных щелчков одиночный
    /// импульс волны 2 читался вдвое сильнее). Двойные импульсы запрещены
    /// навсегда.
    private static let waveform: Int32 = 1
