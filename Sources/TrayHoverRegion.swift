...
enum TrayHover {
    ...
    /// Холодная задержка тултипа команды-иконки. Осознанный выбор, а не
    /// системное значение: help tag ждёт ~1.5s, что для ряда без подписей
    /// слишком медленно; 0.5s не мешает транзитному проезду курсора, а тёплое
    /// перенацеливание делает соседние ярлыки мгновенными.
