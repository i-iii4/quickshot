...
/// Смещение ленты после изменения её длины (`TR-40`).
///
/// Ресайз карточек меняет длину ленты: крупнее карточки — длиннее лента.
/// Абсолютное смещение при этом теряет смысл, потому что оказывается посреди
/// НОВОЙ ленты: собранная колода недособрана и разваливается от малейшей
/// подтяжки за край. Переносится ДОЛЯ раскрытия — собранная остаётся
/// собранной, раскрытая раскрытой на ту же долю.
///
/// Лента без хода считается собранной (`TR-39`), поэтому доля у неё единица.
func trayOffsetPreservingShare(offset: CGFloat,
                               maximum: CGFloat,
                               newMaximum: CGFloat) -> CGFloat {
    let share = maximum > 0.5 ? min(1, max(0, offset / maximum)) : 1
    return max(0, newMaximum) * share
}
