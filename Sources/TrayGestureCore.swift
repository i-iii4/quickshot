...
final class TrayAxisParty: TrayGestureParty {
    ...
    func gestureEnded(_ event: TrayGestureEvent, out: TrayGestureOutput) {
        // Жест кончился — накопленного хода не остаётся. Состояние не
        // переживает событие, которое его завершает: иначе связь неявная —
        // «ход почистит начало следующего жеста», и чтобы понять настоящее,
        // надо помнить о будущем.
        //
        // Обнуление на начале следующего жеста СОХРАНЕНО: конец приходит не
        // всегда (инерция может оборваться), и тогда чистит начало.
        // Инерции это не мешает — ось её не выбирает вовсе, события догона
        // идут по уже выбранной оси.
        picker.restart()
    }
}
...
final class TrayBoundaryParty: TrayGestureParty {
    ...
    func gestureEnded(_ event: TrayGestureEvent, out: TrayGestureOutput) {
        // Признак передачи инерции здесь НЕ сбрасывается, и это не упущение.
        // Отпускание за краем сознательно оставляет инерцию пружине: сбрось
        // его на конце жеста — и первое же событие догона снова потянет ленту
        // наружу, отменив пружину (`TR-13`). Признак снимает либо начало
        // следующего жеста, либо конец самой инерции.
    }
}
