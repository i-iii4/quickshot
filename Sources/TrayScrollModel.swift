...
enum TrayDeckClosure {
    ...
    /// Кривая схлопывания: квадрат доли пройденного пути.
    ///
    /// Форма выбрана НЕ по вкусу, а из условия равномерности сжатия. Зазор
    /// между стопкой и доезжающей карточкой равен остатку пути `d` — это
    /// геометрия ленты, кривой не подчиняется. Чтобы промежутки стопки
    /// сокращались в той же пропорции и ни один слой не пропадал раньше
    /// других, раскрытие обязано быть ЛИНЕЙНЫМ по `d` вблизи посадки.
    /// У квадрата это так: раскрытие `1 - c²` при `c = 1 - d/zone` даёт
    /// `≈ 2d/zone`. Отношение «промежуток яруса к зазору» держится около
    /// 0.2 на всём подходе — слои сближаются, но никуда не деваются.
    ///
    /// Вход в зону гладкий: производная в нуле нулевая, рывка при
    /// пересечении границы нет. Сглаженный шаг `3c² - 2c³` отвергнут
    /// 20.08.2026 — он гладок и у посадки тоже, а значит гасит промежутки
