...
enum TrayDeckClosure {
    ...
    /// Кривая схлопывания: `1 - (1 - c)²`.
    ///
    /// Отвечает на два требования сразу. Отклик МГНОВЕННЫЙ — в начале
    /// скорость максимальна, поэтому «потянул и сразу видно». Приход
    /// МЯГКИЙ — скорость приходит к нулю в точке закрытия, поэтому колода
    /// закрывается, а не обрывается. Прежняя степенная кривая делала
    /// наоборот: разгонялась к концу и вставала колом, с 23 единиц до нуля
    /// за кадр (аудит 20.08.2026).
