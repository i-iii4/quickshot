...
struct TrayDetentModel: Equatable {
    ...
    ///
    /// Значение — ЛЮФТ ЗАЩЁЛКИ, и он обязан дотягиваться до зоны видимого
    /// схлопывания: полный ход страгивания `escape / holdTension` = 37 pt
    /// против 22.4 pt, с которых колода начинает приоткрываться. Иначе
    /// подтягивание до щелчка не показывает ничего, и открытие происходит
    /// разом после срыва — несимметрично постепенному закрытию (аудит
    /// поведения как физического объекта, 20.08.2026). Значение 14 давало
    /// 8 pt и было именно таким; 6 давало 20 pt, тоже недостаточно.
    ///
    /// Цена решения названа прямо: защёлка стала мягче, чем задумывалась
    /// настройкой 19.08.2026, где 20 pt отвергли как «упругую резину
    /// вместо механического зацепа». Приоткрывание при подтягивании —
    /// исходное требование заказчика, и оно физически несовместимо с
    /// коротким зацепом: у мёртвой хватки нечему приоткрываться.
    static let holdTension: CGFloat = 3.0
...
enum TrayDeckClosure {
    ...
    ///
    /// Участок между концом схлопывания и посадкой — ПЛАТО, где степень
    /// закрытия уже единица и на движение ленты не отзывается. Плато
    /// обязано быть длиннее хода щелчка (`snapRemainder` = 14 pt) с
    /// запасом, иначе колода досхлопывается прямо в прыжке и два события
    /// сливаются в одно смазанное (приёмка 20.08.2026).
    ///
    /// Но плато же глушит подтягивание: пока лента идёт по нему, ярусы не
    /// шевелятся. Поэтому значение выбрано МИНИМАЛЬНЫМ из допустимых —
    /// плато 18.8 pt против 14 pt щелчка. Прежние 0.6 давали плато 22.4 pt,
    /// оно съедало две трети люфта защёлки, и приоткрывание появлялось лишь
    /// на последней четверти хода пальца (аудит 20.08.2026).
    ///
    /// Ранний отклик ограничен структурно: плато не может стать короче
    /// щелчка. Сдвинуть границу дальше можно только приблизив щелчок к
    /// посадке, то есть ослабив его, — решение заказчика, не подбор.
    static let completionPoint: CGFloat = 0.665

    /// Кривая схлопывания: сглаженный шаг `3c² - 2c³`.
    ///
    /// Гладкая с ОБОИХ концов: скорость равна нулю и на входе в зону, и в
    /// точке закрытия. Обе односторонние кривые были отвергнуты аудитом
    /// 20.08.2026 — степенная разгонялась к концу и вставала колом, а
    /// `1-(1-c)²` стартовала с максимальной скоростью, и колода дёргалась в
    /// движение при пересечении границы зоны (0.91 pt за полтакта против
    /// обычных 0.3). Максимум скорости теперь в середине хода, где движение
    /// и должно быть самым заметным.
    static func curve(_ c: CGFloat) -> CGFloat {
        let x = min(1, max(0, c))
        return x * x * (3 - 2 * x)
    }
    ...
    static func collapse(presented: CGFloat, model: TrayScrollModel) -> CGFloat {
        curve(value(presented: presented, model: model))
    }
}
