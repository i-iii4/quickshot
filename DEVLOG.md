...
## 20.08.2026 — TR-34: собранная колода закрыта

Эксперимент по запросу: после щелчка сбора видна только верхняя карточка,
задние слои прячутся. До щелчка ярусы видны как раньше — они подсказывают,
что за краем есть содержимое.

Сделано в кадре раскладки: при зацеплённой защёлке из видимых остаётся
слот с наибольшим индексом, остальные уходят в скрытые. Шкатулка строится
по видимым карточкам, поэтому автоматически обнимает одну.

## 20.08.2026 — TR-34: анимация закрытия колоды

Приёмка: концепция верна, но переход был мгновенной сменой состояния.
Сделано по общим правилам трея: ярусы едут к позиции верхней карточки и
гаснут вместе с тенью, пружина критически задемпфированная с откликом
0.3 с. Движение считается от ТЕКУЩЕГО значения закрытия, поэтому щелчок
посреди анимации перенацеливает её, а не рвёт. Слои уходят в скрытые
только по достижении полного закрытия. Reduce motion — затухание без
уезда.

Кадр анимации перекладывает карточки отдельным путём, не трогая состояние
защёлки: иначе анимация перезапускала бы саму себя.

## 20.08.2026 — TR-34: ярусы следуют за жестом, у каждого своя доля пути

Приёмка: «тяну — работает резина, щелчок, и только после этого ярусы».
Требование пользователя: потянул шкатулку — ярус выглядывает сразу,
пропорционально ходу; отпустил, не дойдя до щелчка — всё вернулось.

Сделано: степень закрытия колоды ведёт жест. Пока лента защёлкнута,
закрытие равно `1 - strain/escape`, то есть ровно накопленному ходу
побега; таймерная пружина включается только на разрывах — щелчок посадки,
срыв, программный сбор.

Плюс своя анимация каждому слою: ближний к верхней карточке трогается
первым и первым доходит, дальний позже. Окна соседних ярусов
перекрываются (сдвиг 0.35 от общего пути на всю глубину), поэтому очередь
читается, но движение остаётся слитным.

## 20.08.2026 — TR-34: аудит анимации колоды и переделка

Приёмка: «отпускаю до щелчка — сбрасывается одним кадром; неравномерно;
слишком много движения до щелчка». Аудит собственных решений дал три
промаха:

1. До щелчка ярусы раскрывались ПОЛНОСТЬЮ по ходу пальца — щелчку не
   оставалось движения. Теперь жесту отдана треть раскрытия, остальное
   делает щелчок.
2. Окна слоёв были разной длины (у дальнего короче), поэтому слои шли с
   разной скоростью — это и читалось как неравномерность. Теперь окна
   равные, сдвинуты по глубине на пятую часть пути: одна скорость, разный
   старт.
3. Возврат недожатого выхода доводится той же пружиной, что и позиция
   ленты, и всегда анимирован.
