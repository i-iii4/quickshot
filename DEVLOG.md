...
## 19.08.2026 (ночь-10) — прокрутка отдана системе

Пользователь остановил подбор параметров вопросом по существу: почему мы
проектируем поведение, которое уже есть в операционной системе, и почему
он должен участвовать в изобретении велосипеда. Оба вопроса справедливы.

Ответ по коду: трей — одна панель с обычными вью внутри, поэтому системный
`NSScrollView` можно использовать как ДВИГАТЕЛЬ. События прокрутки уходят
в него, он двигает свой пустой документ с родной физикой, а трей на каждое
изменение читает смещение и раскладывается прежним кодом. Карточки внутрь
не переносятся — раскладка со стопками остаётся своей.

Выброшено: формула сопротивления с подобранными числами, отложенный
возврат, флаги активного жеста и анимации возврата, накопители защёлки
(удержание, побег, осадка), пружина осадки.

Защёлка переписана как чистое преобразование координат: ход системного
скролла длиннее видимого на растянутую зону напряжения, поэтому последние
56 pt ленты проходят вчетверо медленнее пальца. Щелчок срабатывает на
переходе через порог, движение ведёт система. Тесты: прозрачность вне
зоны, равномерность натяжения внутри, обратимость и монотонность
преобразования, отсутствие защёлки у коротких лент.

Почему я не сделал так сразу: унаследовал самодельную модель прокрутки от
более ранней работы в этой сессии и правил её, ни разу не спросив, даёт ли
платформа готовое поведение. Плюс, взяв настоящую формулу Apple, подставил
в неё неверную величину и вместо проверки подстановки заменил параметры на
выдуманные.
