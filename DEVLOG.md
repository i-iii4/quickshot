...
## 20.08.2026 — TR-34: закрытие колоды как функция позиции

После двух неудачных подходов (своя пружина у ярусов, деление пути между
жестом и щелчком) требования переформулированы и согласованы, реализация
сделана заново с нуля.

Ключ: степень закрытия — чистая функция видимой позиции ленты (смещение
плюс подача защёлки). Никакого своего состояния, аниматоров и таймеров у
ярусов нет. Отсюда все требования выполняются по построению: движение
непрерывно, потому что непрерывна позиция; жест ведёт ярусы до щелчка;
щелчок доводит их вместе с лентой; отмена возвращает тем же путём;
прерывание работает само, потому что нечего перезапускать.

Очередь слоёв — сдвиг окон по глубине на пятую часть пути при одинаковой
длине окна.

## 20.08.2026 — TR-34: срыв больше не телепортирует ленту

Приёмка: после щелчка резкий скачок — верхняя карточка вверх, ярусы вниз,
одним движением. Проверка кода нашла источник: на срыве позиция
присваивалась разом (`maxOffset - zone`, около 36 pt). Подача этот прыжок
гасила, но за 120 мс — и в эти 120 мс укладывалось ВСЁ раскрытие колоды.

Убран сам прыжок: срыв снимает зацеп на месте, дальше лента идёт за
пальцем один к одному. Щелчок остаётся тактильным и меняет режим
сопротивления — натяжение вчетверо сменяется свободным ходом. Раскрытие
колоды теперь целиком следует за пальцем, как того требует правило о
непрерывности.
## 20.08.2026 — TR-34: колода закрывается наезжанием

Разбор с заказчиком показал, что уезд ярусов под карточку был моей
отсебятиной: в требованиях речь шла только о видимости. Из двух приёмов
выбрано наезжание — верхняя карточка съезжает к краю и накрывает ярусы.

Обоснование: носителем движения в жесте является только верхняя карточка,
поэтому наезжание использует существующее движение, а втягивание требует
синтетического движения статичных слоёв — оно и давало рывки во всех трёх
предыдущих подходах. Плюс пространственная честность (слои на местах,
просто заслонены) и метафора шкатулки: крышка накрывает содержимое.

Реализация вышла минимальной: сдвиг верхней карточки к краю на высоту
ярусов, пропорциональный степени закрытия; ярусы не трогаются вовсе.
Очередь слоёв и их затухание удалены за ненадобностью.
