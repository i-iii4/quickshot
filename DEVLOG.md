...
## 20.08.2026 — TR-33: зеркальный сбор стопки у дальнего края

Тестовая версия по запросу заказчика: стопка собирается не только у
ближнего края, но и у дальнего — тянешь ленту вверх, она защёлкивается
зеркальной стопкой.

Раскладка это уже умела: `TrayStripLayout.bands` строит дальнюю стопку для
карточек, чей верх дошёл до дальнего яруса. Не хватало двух вещей:
- хода ленты в обратную сторону: добавлен `minimumOffset` — отрицательное
  смещение до парковки ПЕРВОЙ карточки у дальнего края; у короткого экрана
  он нулевой;
- зеркальной защёлки: `TrayDetentModel` параметризован концом ленты
  (`End.near` / `End.far`). Механика не продублирована — вся математика
  переписана в терминах «путь к посадке» и знака направления, поэтому
  ближний конец работает ровно как раньше.

Тесты: обновлены проверки резинки и осадки (ноль больше не край — край
теперь зеркальная парковка), добавлены зеркальные щелчок посадки и выход
с побегом.
