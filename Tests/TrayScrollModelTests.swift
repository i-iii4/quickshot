...
struct TrayScrollModelTests {
    ...
    /// Натяжение второй ступени (`TR-41`): карточка чуть пружинит, копится
    ...
    private static func stowTensionBuildsWithoutADeadZone() {
        ...
        // Убывание есть: первая четверть хода даёт больше последней.
        ...
        // Но мёртвой зоны нет: последняя четверть даёт заметную долю.
        ...
        // Масштаб идёт вместе со сдвигом и приходит в свою точку.
        ...
        // Порог: только полный ход или намерение броска.
        ...
        // Схлопывание: прозрачность гаснет, масштаб уходит вглубь.
        expect(TrayStow.stowedOpacity(progress: 0) == 1, "начало схлопывания уже прозрачно")
        expect(TrayStow.stowedOpacity(progress: 1) == 0, "конец схлопывания не прозрачен")
        expect(abs(TrayStow.stowedScale(progress: 1, fromScale: TrayStow.tensionScale)
                   - TrayStow.stowedScale) < 0.001,
               "масштаб убранной карточки не дошёл до своей точки")
    }
